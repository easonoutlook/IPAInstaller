...
- (void) loadView {
	...
	_backgroundView = [[UIImageView alloc] init];//WithImage:[UIImage imageNamed:kGGTwitterLoadingBackgroundImage]];
	...
	UINavigationItem *navItem = [[[UINavigationItem alloc] initWithTitle: NSLocalizedString(@"Twitter Account Info", @"Twitter 账户信息")] autorelease];
...
- (UIToolbar *) pinCopyPromptBar {
	if (_pinCopyPromptBar == nil){
		...
		_pinCopyPromptBar.items = [NSArray arrayWithObjects: 
							  ...
							  [[[UIBarButtonItem alloc] initWithTitle: NSLocalizedString(@"Select and Copy the PIN", @"选择并拷贝 PIN") style: UIBarButtonItemStylePlain target: nil action: nil] autorelease], 
							  [[[UIBarButtonItem alloc] initWithBarButtonSystemItem: UIBarButtonSystemItemFlexibleSpace target: nil action: nil] autorelease], 
							nil];
	}
